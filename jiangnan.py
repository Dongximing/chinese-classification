...
from model import RankingLoss, Refactor, CrossEncoder
import math
import logging
...
def base_setting(args):
    ...
    args.accumulate_step = getattr(args, "accumulate_step", 6)
    ...
    args.datatype = getattr(args, "datatype", "beam")
    args.dataset = getattr(args, "dataset", "CNNDM")
    args.use_ids = getattr(args, "use_ids", False)  # set true for pretraining
...
def evaluation(args):
    ...
    if args.use_ids:
        ...
        test_set = RefactoringIDsDataset(f"./{args.dataset}/{args.datatype}/test", args.model_type, is_test=True,
                                         maxlen=512, is_sorted=False)
    else:
        collate_fn = partial(collate_mp, pad_token_id=tok.pad_token_id, is_test=True)
        test_set = RefactoringDataset(f"./{args.dataset}/{args.datatype}/test", args.model_type, is_test=True,
    ...
    with torch.no_grad():
        for (i, batch) in enumerate(dataloader):
            ...
            samples = batch["data"]
            output = model(batch["tgt_input_ids"], batch["candidate_ids"], batch["tgt_input_ids"])
    ...
    with torch.no_grad():
        for (i, batch) in enumerate(dataloader):
            if args.cuda:
                to_cuda(batch, gpuid)
            samples = batch["data"]
            similarity = model(batch["input_ids"], batch["attention_mask"], batch["segment_ids"])
            similarity = similarity.cpu().numpy()
            if i % 1000 == 0:
                print(f"test similarity: {similarity[0]}")
            max_ids = similarity.argmax(1)
...
def run(rank, args):
    ...
    tok = BertTokenizer.from_pretrained("./bert_base/")
    if args.use_ids:
        ...
        train_set = RefactoringIDsDataset(f"./{args.dataset}/{args.datatype}/train", args.model_type,
                                          maxlen=args.max_len, max_num=args.max_num)
        val_set = RefactoringIDsDataset(f"./{args.dataset}/{args.datatype}/val", args.model_type, is_test=True,
                                        maxlen=512, is_sorted=False)
    else:
        ...
        train_set = RefactoringDataset(f"./{args.dataset}/{args.datatype}/train", args.model_type, maxlen=args.max_len,
                                       maxnum=args.max_num)
        val_set = RefactoringDataset(f"./{args.dataset}/{args.datatype}/val", args.model_type, is_test=True, maxlen=512,
    ...
    for epoch in range(args.epoch):
        ...
        for (i, batch) in enumerate(dataloader):
            ...
            loss = args.scale * RankingLoss(similarity, gold_similarity, args.margin, args.gold_margin,
                                            args.gold_weight, no_gold=args.no_gold)
